...
void D3DApp::updateScene(float dt){
	...
	if(GetAsyncKeyState('W') & 0x8000)	GetCamera().MoveForward();
	if(GetAsyncKeyState('S') & 0x8000)	GetCamera().MoveBack();
	if(GetAsyncKeyState('Z') & 0x8000)	GetCamera().MoveUp();
	...
	mLights[1].pos.y = 4.0f;

	mLights[2].pos = GetCamera().mPosition;
	D3DXVec3Normalize(&mLights[2].dir,&(GetCamera().mForward-GetCamera().mPosition));
}
